...
#pragma omp parallel for reduction (+:cor)
    ...
    RealMatrix<N,K> res(ResMatSize,K);
#pragma omp parallel for
    for (unsigned int i = 0; i < ResMatSize; i++) {
...
template<std::size_t N>
RealVector<N> trapped_ion_mode_pattern()
{
    RealMatrix<N,N> jmat(N,N);
#include JMAT_INC
    static_assert(sizeof(Jmat) == N * N * sizeof(RealScalar));
    for (unsigned int i = 0; i < N; i++) {
	for (unsigned int j = 0; j < N; j++) {
	    jmat(i,j) = (Jmat[i][j] + Jmat[j][i]) / 2.0;
	}
    }

    Eigen::SelfAdjointEigenSolver<RealMatrix<N,N>> solver(jmat);
    RealVector<N> pattern(N);
    auto vec = solver.eigenvectors().col(0);
    for (unsigned int i = 0; i < N; i++) {
	pattern[i] = vec[i] >= 0 ? 1.0 : -1.0;
    }
    return pattern;
}
...
#pragma omp parallel for collapse(3)
    ...
    auto classty = sizeof(RealScalar) == 4 ? MAT_C_SINGLE : MAT_C_DOUBLE;
    auto dataty = sizeof(RealScalar) == 4 ? MAT_T_SINGLE : MAT_T_DOUBLE;
    auto write_var = [&](const char *name, int rank, size_t *dims, RealScalar *data) {
	matvar_t *mv = Mat_VarCreate(name, classty, dataty, rank, dims, data, 0);
	if (!mv) {
	    ThrowException(MatioCreateVarError, name);
	}
	Mat_VarWrite(mf, mv, MAT_COMPRESSION_NONE);
	Mat_VarFree(mv);
    };
    ...
    size_t dims1[] = { N, K };
    write_var("x", 2, dims1, x.data());
    write_var("z", 2, dims1, z.data());

    size_t dims[] = { N, N, K };
    write_var("xx", 3, dims, xx.data());
    write_var("zz", 3, dims, zz.data());

    RealVector<N> pattern = trapped_ion_mode_pattern<N>();
    std::vector<RealScalar> mode_pattern(N), mx(K), mz(K), mz2(K), mode_z(K), mode2(K);
    for (unsigned int i = 0; i < N; i++) {
	mode_pattern[i] = pattern[i];
    }
    for (unsigned int k = 0; k < K; k++) {
	for (unsigned int i = 0; i < N; i++) {
	    mx[k] += corx(i,k) / N;
	    mz[k] += corz(i,k) / N;
	    mode_z[k] += pattern[i] * corz(i,k) / N;
	    for (unsigned int j = 0; j < N; j++) {
		RealScalar zz_ij = corzz(i*N+j,k);
		mz2[k] += zz_ij / (N * N);
		mode2[k] += pattern[i] * pattern[j] * zz_ij / (N * N);
	    }
	}
    }
    size_t dimsN[] = { N, 1 };
    size_t dimsK[] = { K, 1 };
    write_var("mode_pattern", 2, dimsN, mode_pattern.data());
    write_var("mx", 2, dimsK, mx.data());
    write_var("mz", 2, dimsK, mz.data());
    write_var("mz2", 2, dimsK, mz2.data());
    write_var("mode_z", 2, dimsK, mode_z.data());
    write_var("mode2", 2, dimsK, mode2.data());
    Mat_Close(mf);
}
...
struct CmdArgs : public argparse::Args {
    ...
    bool &apply_gauge = flag("apply-gauge", "Apply the local gauge transformation "
			     "a_i -> a_i - I PI / 2 for odd i when loading weights "
			     "and before dumping weights.");
    bool &inverse_gauge = flag("inverse-gauge", "When loading with --apply-gauge, use the "
			     "inverse sign a_i -> a_i + I PI / 2 for odd i.");
    std::string &correlation_data = kwarg("correlation-data", "Specify the file name to save the "
					  "correlation function data. If omitted, "
					  "correlation data are not written.").set_default("");
...
void run(Args &&args, auto &&hamiltonian)
{
    ...
    RealVector<K> res{RealVector<K>::Constant(K, std::numeric_limits<RealScalar>::max())};
    Phi<K> e_mat(K,K);
	...
	if ((evals - res).norm() < args.tol) {
	    std::cout << "Converged after " << iter+1 << " steps." << std::endl;
	    break;
	}
	if (evals.sum() < res.sum()) {
	    res = evals;
	}
    }
    ...
    if (!args.correlation_data.empty()) {
